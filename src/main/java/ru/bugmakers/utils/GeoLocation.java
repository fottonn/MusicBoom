...
public class GeoLocation {
    ...
    private static final double EARTH_RADIUS = 6371010;
    ...
    public double distanceTo(GeoLocation location) {
        return Math.acos(Math.sin(radLat) * Math.sin(location.radLat) +
                Math.cos(radLat) * Math.cos(location.radLat) *
                        Math.cos(radLon - location.radLon)) * EARTH_RADIUS / 1000;
    }
     ...
     * argument.</p>
     *
     * @param radius the distance from the point represented by this
     ...
     * </ul>
     */
    public GeoLocation[] boundingCoordinates(double radius) {
